...
def plot_param_search_results_win_all():
    ...
    for expert in range(1, 4):
        ...
        total_accuracy_array.append(total_accuracy_array_per_expert)

    total_accuracy_array = np.array(total_accuracy_array)
    total_accuracy_array = np.mean(total_accuracy_array, axis=1)
    plt.figure(figsize=(9,5))
    y1 = total_accuracy_array[0, :]
    y2 = total_accuracy_array[1, :]
    y3 = total_accuracy_array[2, :]
    plt.plot(windows,y1, 'bo-', label='expert 1')
    plt.plot(windows,y2, 'r^-.', label='expert 2')
    plt.plot(windows,y3, 'gs--', label='expert 3')
    ...
    plt.grid()
    plt.tight_layout()
    plt.savefig(path_variables.FIGURES_PATH + 'param_search_win_5_folds_5_runs_all.pdf')
    plt.show()
    plt.close()

if __name__ == "__main__":
    print("Usage: python plot_param_search_results_win_all.py")
    for expert in range(1, 4):
        if not utilities.all_results_win_search_exist(expert):
            print(
                f"csv files containing optimal window search results for a given expert {expert} are missing in RESULTS folder. Please run param_search_win.py")
            sys.exit(1)
    plot_param_search_results_win_all()
