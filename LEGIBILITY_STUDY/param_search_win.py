...
def param_search_win(expert):
    ...
    # min sample size 1774
    # max sample size 4844
    # np.arange(64,1774,64)
    for run in range(NUM_RUNS):
        RANDOM_STATE = RANDOM_STATE_PER_RUN[run]
        ros = RandomOverSampler(random_state=RANDOM_STATE)

        report_filename = path_variables.RESULTS_PATH + "param_search_win_expert_" + str(expert) + "_run_" + str(
            run) + ".csv"
        for win_size in np.arange(MIN_WIN, MAX_WIN, WIN_STEP):
            utilities.print_gpu_usage()
            overlap = int(win_size * OPT_OVR)
            #################################################################################################################
            #
            ...
            for ifold in range(N_FOLDS):
                ...
                print(f"Running fold {ifold} run {run} for expert {expert} for window {win_size}")
                ...
                reports_df.loc[len(reports_df)] = [win_size, OPT_OVR, EPOCHS, LR, BATCH_SIZE, DROPOUT,
                                                   report["accuracy"],
                                                   avg_precision,
...
if __name__ == "__main__":
    ...
    if not utilities.all_results_win_search_exist(expert_arg):
        param_search_win.param_search_win(expert_arg)
        print(f"Search for optimal window for expert {expert_arg} is complete. Results are saved.")
    else:
